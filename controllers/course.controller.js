    ...
    try {
        ...
        if (categories) {
            if (typeof categories === "string") {
                categoriesArr = categories.split(",").map(cat => cat.trim());
        ...
        if (categoriesArr.length > 0) {
            searchCriteria.category = { $in: categoriesArr };
        }

        // define sorting order
        const sortOptions = {};
        if (sortByPrice === "low") {
            sortOptions.coursePrice = 1; // ascending price
        } else if (sortByPrice === "high") {
            sortOptions.coursePrice = -1; // descending price
        }
...
export const togglePublishCourse = async (req, res) => {
    try {
        ...
        if (!["true", "false"].includes(publish)) {
            return res.status(400).json({
                message: "Invalid value for publish parameter"
            });
        }
        ...
        return res.status(200).json({
            course,
            message: isPublish ? "Course published successfully" : "Course unpublished successfully"
        });
    } catch (error) {
        console.log(error);
        return res.status(500).json({
